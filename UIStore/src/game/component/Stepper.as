package game.component
{
	...
	public class Stepper extends Container
	{
		...
		/**
		 * 修改成自定义的value，主要是处理step不是1的情况
		 */
		public function setUserDefineValue(val:Number):void
		{
			_value = val;
		}
		...
		private function removeAllEventListener():void
		{
			...
			if(_prevBtn)
			{
				_prevBtn.removeEventListener(MouseEvent.CLICK, onBtnClick);
			}
			if(_nextBtn)
			{
				_nextBtn.removeEventListener(MouseEvent.CLICK, onBtnClick);
			}
			if(_firstBtn != null)
			{
				_firstBtn.addEventListener(MouseEvent.CLICK, onBtnClick);
